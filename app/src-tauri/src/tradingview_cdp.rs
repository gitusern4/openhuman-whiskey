...
pub async fn tv_cdp_eval(
    ...
    tv_cdp_eval_internal(state, expression).await
}

/// Internal eval helper — runs the JS expression without the webview-
/// label gate. Used by sibling Tauri commands in this file
/// (`tv_cdp_get_chart_state`, `tv_cdp_set_symbol`, `tv_cdp_draw_sltp`,
/// `tv_cdp_clear_sltp`, `tv_cdp_get_order_flow_state`) that have
/// already passed their own validation and execute trusted, hard-
/// coded JS snippets — not caller-supplied expressions. Do NOT expose
/// this directly to IPC; it deliberately has no webview gate.
pub(crate) async fn tv_cdp_eval_internal(
    state: tauri::State<'_, TvCdpState>,
    expression: String,
) -> Result<Value, String> {
    let mut guard = state.0.lock().await;
...
pub async fn tv_cdp_get_chart_state(
    ...
    let raw = tv_cdp_eval_internal(state, JS_GET_CHART_STATE.to_string()).await?;
...
pub async fn tv_cdp_set_symbol(
    ...
    let raw = tv_cdp_eval_internal(state, expr).await?;
    let parsed: Value = match &raw {
        Value::String(s) => serde_json::from_str(s).unwrap_or(Value::Null),
        other => other.clone(),
    };
...
pub async fn tv_cdp_clear_sltp(
    ...
    let raw = tv_cdp_eval_internal(state, JS_CLEAR_SLTP.to_string()).await?;
    let parsed: Value = match &raw {
        Value::String(s) => serde_json::from_str(s).unwrap_or(Value::Null),
        other => other.clone(),
    };
...
pub async fn tv_cdp_get_order_flow_state(
    ...
    let raw = match tv_cdp_eval_internal(state, JS_GET_ORDER_FLOW_STATE.to_string()).await {
        Ok(v) => v,
        Err(e) => {
